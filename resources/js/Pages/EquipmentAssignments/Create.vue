...
const submit = () => form.post(route('equipment-assignments.store'));
...
<template>
    <Head title="Nouvelle fiche d'affectation" />

    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-3">
                <Link :href="route('equipment-assignments.index')" class="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700">
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                    </svg>
                </Link>
                <div>
                    <p class="text-xs font-semibold uppercase tracking-wide text-primary">DL-FTE-01</p>
                    <h1 class="text-2xl font-bold text-slate-900">Nouvelle fiche d'affectation</h1>
                </div>
            </div>
        </template>
        ...
        <div class="mx-auto max-w-4xl">
            <form @submit.prevent="submit" class="space-y-6">
                <section class="card overflow-hidden">
                    ...
                    <div class="grid grid-cols-1 gap-5 p-6 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <TextInput id="equipment_mac" v-model="form.equipment_mac" type="text" class="mt-1.5" placeholder="XX:XX:XX:XX:XX:XX" />
                    ...
                    <div class="space-y-5 p-6">
                        <div class="rounded-xl border border-dashed border-primary-200 bg-primary-50/40 p-4">
                            ...
                            <p class="mt-2 text-xs text-slate-500">
                                💡 Les champs ci-dessous se rempliront automatiquement à la sélection — vous pouvez toujours les modifier.
                            </p>
                        </div>
                ...
                <div class="flex items-center justify-end gap-3">
                    <SecondaryButton type="button" @click="$inertia.visit(route('equipment-assignments.index'))">Annuler</SecondaryButton>
